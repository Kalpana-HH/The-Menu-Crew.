...
export default function AuthScreen({ onAuthSuccess }: AuthScreenProps) {
  ...
  // Google Sign-In prompt modal states
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.needsGooglePrompt) {
        setGooglePromptEmail(email);
        setGooglePromptName(name);
  ...
  const handleConfirmGoogleDetails = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!googlePromptEmail.trim() || !googlePromptEmail.includes('@')) {
    ...
    try {
      const user = await loginWithGoogle(
        role, 
        googlePromptEmail.trim(), 
        googlePromptName.trim() || undefined
      );
      if (user.email && user.email.includes('@')) {
        sendAutomatedEmail({
          to: user.email,
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] text-neutral-800 flex flex-col justify-center items-center p-4 font-sans selection:bg-[#C88A8A]/20 selection:text-neutral-900" id="auth-screen-root">
      ...
      {/* Google Details Modal (Triggered when Google popup is restricted or blocked on custom domain) */}
      {showGooglePromptModal && (
        <div className="fixed inset-0 bg-neutral-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 font-sans">
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="bg-white border border-[#EBE7DF] rounded-3xl w-full max-w-md shadow-2xl overflow-hidden p-6 sm:p-8 space-y-5"
          >
            ...
            <div className="bg-amber-50/80 border border-amber-200/80 rounded-2xl p-3.5 space-y-1.5 text-xs">
              <p className="font-bold text-amber-900 flex items-center gap-1.5">
                <span>ℹ️ Domain Authorization Note</span>
              </p>
              <p className="text-amber-950 text-[11px] leading-relaxed">
                Native Google popup OAuth is restricted on <strong>{typeof window !== 'undefined' ? window.location.hostname : 'your custom domain'}</strong>.
                To enable direct Google OAuth popups on Vercel, ensure <strong>{typeof window !== 'undefined' ? window.location.hostname : 'domain'}</strong> is added to:
              </p>
              <ul className="list-disc list-inside text-[10px] text-amber-900 font-mono space-y-0.5 pl-1">
                <li>Firebase Console &gt; Authentication &gt; Settings &gt; Authorized Domains</li>
                <li>Google Cloud Console &gt; OAuth 2.0 Client ID &gt; Authorized Origins</li>
              </ul>
            </div>

            <form onSubmit={handleConfirmGoogleDetails} className="space-y-3.5">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Kalpana Anumula"
                  className="w-full px-3.5 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl text-neutral-800 text-xs focus:bg-white focus:outline-hidden focus:ring-1 focus:ring-[#C88A8A]"
                  required
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. kalpana@gmail.com"
                  className="w-full px-3.5 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl text-neutral-800 text-xs focus:bg-white focus:outline-hidden focus:ring-1 focus:ring-[#C88A8A]"
                  required
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="flex-1 py-2.5 px-3 bg-neutral-100 hover:bg-neutral-200 text-neutral-700 text-xs font-semibold rounded-xl transition cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 py-2.5 px-3 bg-[#C88A8A] hover:bg-[#B57878] text-white text-xs font-bold rounded-xl transition cursor-pointer shadow-xs"
                >
                  Continue to App
                </button>
              </div>
